#!/bin/bash
# Script lancé en fin d'installation preseed Debian jessie
# pour finaliser de la config .
#
# 
# lastupdate 05-06-2014
...
wget -O $POST/02-mesapplis-debian-lxde.txt http://192.168.98.5/install/02-mesapplis-debian-lxde.txt
...
if [ -n "${ip_se3}" ]; then
    ...
    if [ "$?" = "0" ]; then
        ...
        chmod +x integration_jessie.bash
    else
...
while [ -z "$nom_machine" ]
    do
        echo "Machine non connue de l'annuaire, Veuillez saisir un nom"
