...
# Choisir l'environnement des clients lourds Trusty : ubuntu, xubuntu ou lubuntu
...
echo "------------------------------------------------------------------------------------------------------------------------------"
echo " Ce script installe un serveur LTSP de clients lourds Trusty sur votre SE3 Wheezy												"
...
echo " - créer un répertoire /opt/ltsp/lubuntu (le chroot) contenant la racine / des clients lourds trusty							"
echo " - installer et configurer les services NFS et NBD pour distribuer l'environnement (le chroot) des clients lourds 			"
echo " - créer un répertoire /tftpboot/ltsp contenant l'initrd et le kernel pour le boot PXE des clients lourds	trusty 				"
echo " - ajouter une entrée au menu /tftpboot/pxelinux.cfg/default pour pouvoir démarrer un PC PXE en client lourd Trusty 			"
echo " - configurer le chroot des clients lourds trusty pour l'identification avec l annuaire ldap et le montage automatique des partages Samba du se3  "
echo "------------------------------------------------------------------------------------------------------------------------------"
...
echo "------------------------------------------------------------------------------------------------------------------------------"
echo " 2-Construction de l environnement $ENVIRONNEMENT pour les clients lourds Trusty												"
echo "------------------------------------------------------------------------------------------------------------------------------"
echo " ATTENTION : la construction du chroot trusty change la locale du serveur ...													"
...
# Le script de construction du chroot de Trusty n'existe pas dans le paquet ltsp de wheezy
# Mais en regardant le paquet ltsp pour Jessie, on remarque que c'est simplement un lien vers le script gutsy
# Et en testant gutsy sous wheezy, on remarque que le script est fonctionnel pour trusty ...

if [ ! -e "/usr/share/debootstrap/scripts/trusty" ]
then
...
echo "------------------------------------------------------------------------------------------------------------------------------"
echo " 5- Configuration du menu PXE du se3 afin d ajouter une entrée pour pouvoir démarrer un PC PXE en client lourd Trusty 	    "
echo "------------------------------------------------------------------------------------------------------------------------------"
cat <<EOF >> "/tftpboot/pxelinux.cfg/default"
LABEL ltspTrusty
	MENU LABEL ^Demarrer le pc en client lourd Trusty $ENVIRONNEMENT
...
echo "--------------------------------------------------------------------------------------"
echo " 11-Configuration de l environnement $ENVIRONNEMENT des clients lourds Trusty	 		"
echo "--------------------------------------------------------------------------------------"
...
echo "--------------------------------------------------------------------------------------"
echo " 13-Reconstruction de l'image squashfs (spécifique à Trusty avec NBD)					"
echo "--------------------------------------------------------------------------------------"
