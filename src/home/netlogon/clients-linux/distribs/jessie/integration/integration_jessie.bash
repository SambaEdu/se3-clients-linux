...
modif_bloc_polkit()
{
    ...
    sed -i '/'$1'\"/,/\/allow_active/ s/yes/no/' $fichier_polkit
    sed -i '/'$1'\"/,/\/allow_any/ s/auth_admin_keep/no/' $fichier_polkit
    sed -i '/'$1'\"/,/\/allow_inactive/ s/auth_admin_keep/yes/' $fichier_polkit 
}
...
configurer_polkit()
{
    ...
    # on teste si le fichier polkit a déjà été modifié
    if [ "$(cat "$fichier_polkit" | grep modification_intégration)" = "" ]
    then
        ...
        # on rajoute un commentaire pour l'idempotence
        cat >> "$fichier_polkit" << END

# modification_intégration
END
    fi
}
