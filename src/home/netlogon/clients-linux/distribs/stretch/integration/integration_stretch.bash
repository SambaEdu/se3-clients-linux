...
##### #####
# script d'intégration des clients Stretch à un domaine géré par un se3
#
#
# version : 20170823
#
...
#SORTIE="/dev/null"
SORTIE="/root/log_debug_stretch_${ladate}.txt"
...
rechercher_ldap_client()
{
    ...
    # Liste des cartes réseau en éliminant lo
    cartes_reseau=$(ip -4 -o addr show | cut -d" " -f 2 | grep -v "lo")
    
    # cas sans carte réseau…
    if [ $cartes_reseau = "" ]
    then
        echo "pas de carte réseau ?"
        # gestion de ce cas ? [TODO]
    else
        ...
        for i in $cartes_reseau
        do
            carte=$i
            adresse_mac=$(ip -4 link show $i | tr -s ' ' | grep link | cut -d" " -f3)
            adresse_ip=$(ip -4 -o addr show $i | tr -s ' ' | cut -d" " -f4 | cut -d"/" -f1)
...
END
    ...
    # à partir de stretch, c'est un service ? [TODO]
