...
# On vérifie que le système est bien Debian Squeeze.
if [ "$NOM_DE_CODE" != "squeeze" ]; then
    afficher "Désolé, le script doit être exécuté sur Debian Squeeze." \
             "Fin du script."
    exit 1
fi
...
# du système.
invoke-rc.d avahi-daemon stop >/dev/null 2>&1
...
invoke-rc.d exim4 stop >/dev/null 2>&1
update-rc.d -f exim4 remove >/dev/null 2>&1
...
if "$OPTION_MDP_GRUB"; then
    ...
    invoke-rc.d rng-tools stop > /dev/null 2>&1
    invoke-rc.d rng-tools start > /dev/null 2>&1
...
invoke-rc.d networking stop > /dev/null 2>&1
invoke-rc.d networking start > /dev/null 2>&1
...
apt-get install --yes  sudo > /dev/null 2>&1
...
invoke-rc.d nslcd stop > /dev/null 2>&1
invoke-rc.d nslcd start > /dev/null 2>&1
...
########################################################
### Modification de /etc/gdm3/greeter.gconf-defaults ###
########################################################
...
restaurer_via_save "/etc/gdm3/greeter.gconf-defaults"
echo "
