...
# Le nom de code de la distribution (par exemple "squeeze" ou "precise").
...
CREDENTIALS="$REP_TMP_LOCAL/credentials"'_$username'
LIGHTDM_CONF="/etc/lightdm/lightdm.conf"

# Les options de base pour un montage CIFS.
OPTIONS_MOUNT_CIFS_BASE="nobrl,serverino,iocharset=utf8,sec=ntlmv2"

# Regex concernant deux lignes du fichier $LIGHTDM_CONF.
MOTIF_GREETER_SESSION='^[[:space:]]*greeter-session[[:space:]]*='
MOTIF_USER_SESSION='^[[:space:]]*user-session[[:space:]]*='
...
{
    { echo "$1"; echo "$1"; } \
        | grub-mkpasswd-pbkdf2 -c 30 -l 30 -s 30 2>/dev/null \
        | grep -v 'password' \
...
function nettoyer_avant_de_sortir ()
{
    case "$?" in
        ...
        "1")
            ...
            apt-get purge --yes $PAQUETS_TOUS >/dev/null 2>&1
            ;;
...
# On vérifie que le fichier LIGHTDM_CONF est correct.
c1=$(grep -ciE "$MOTIF_GREETER_SESSION" "$LIGHTDM_CONF")
c2=$(grep -ciE "$MOTIF_USER_SESSION" "$LIGHTDM_CONF")
if [ "$c1" != "1" ] || [ "$c2" != "1" ]; then
    afficher "Désolé, mais le fichier '$LIGHTDM_CONF' n'est pas correct car" \
             "les paramètres 'user-session' et 'greeter-session' devraient" \
             "apparaître une fois exactement, or ce n'est pas le cas." \
             "Fin du script."
    exit 1
else
    PARAM_GREETER_SESSION=$(grep -iE "$MOTIF_GREETER_SESSION" "$LIGHTDM_CONF" | cut -d'=' -f'2')
    PARAM_GREETER_SESSION=$(echo $PARAM_GREETER_SESSION) # pour faire un strip()
    PARAM_USER_SESSION=$(grep -iE "$MOTIF_USER_SESSION" "$LIGHTDM_CONF" | cut -d'=' -f'2')
    PARAM_USER_SESSION=$(echo $PARAM_USER_SESSION) # pour faire un strip()
    if [ -z "$PARAM_GREETER_SESSION" ] || [ -z "$PARAM_USER_SESSION" ]; then
        afficher "Désolé, mais le fichier '$LIGHTDM_CONF' n'est pas correct car" \
                 "(au moins) une des valeurs des paramètres 'user-session' et" \
                 "'greeter-session' semble vide. Fin du script."
        exit 1
    fi
fi
unset -v c1 c2
...
for paquet in $PAQUETS_TOUS; do
    if ! apt-get install "$paquet" --yes --simulate >/dev/null 2>&1; then
...
apt-get remove --purge --yes libnss-mdns >/dev/null 2>&1
...
if ! ping -c 5 -W 2 "$SE3" >/dev/null 2>&1; then
...
# désintallé.
apt-get purge --yes $PAQUETS_TOUS samba >/dev/null 2>&1
...
# inutile ici.
apt-get install --no-install-recommends --reinstall --yes $PAQUETS_MONTAGE_CIFS >/dev/null 2>&1

# Montage du partage NOM_PARTAGE_NETLOGON.
mkdir "$REP_NETLOGON"
chown "root:root" "$REP_NETLOGON"
chmod 700 "$REP_NETLOGON"
mount -t cifs "$CHEMIN_PARTAGE_NETLOGON" "$REP_NETLOGON" -o ro,guest,"$OPTIONS_MOUNT_CIFS_BASE" >/dev/null 2>&1
if [ "$?" != "0" ]; then
    rmdir "$REP_NETLOGON"
...
# En fait, le fichier $LIGHTDM_CONF n'est pas le même suivant qu'on est
# sur Ubuntu ou Xubuntu. Alors on va « ruser » et recréer le fichier
# de sauvegarde « à la main ».
echo "
[SeatDefaults]
greeter-session=$PARAM_GREETER_SESSION
user-session=$PARAM_USER_SESSION
" > "${REP_SAVE_LOCAL}$LIGHTDM_CONF"
...
apt-get install --no-install-recommends --reinstall --yes "$PAQUETS_CLIENT_LDAP" >/dev/null 2>&1

# Vérification de la connexion LDAP avec le Se3.
ldapsearch -xLLL -h "$SE3" -b "ou=Computers,$BASE_DN" "(|(uid=$NOM_CLIENT$)(cn=$NOM_CLIENT))" "dn" > /dev/null 2>&1
if [ "$?" != 0 ]; then
...
if "$OPTION_NOM_CLIENT"; then
    ...
    service hostname stop > /dev/null 2>&1
    service hostname start > /dev/null 2>&1
fi
...
if "$OPTION_MDP_GRUB"; then
    ...
    apt-get install --reinstall --yes --force-yes $PAQUETS_RANDOM > /dev/null 2>&1
    echo "HRNGDEVICE=/dev/urandom" >> "/etc/default/rng-tools"
    service rng-tools stop > /dev/null 2>&1
    service rng-tools start > /dev/null 2>&1
    ...
    apt-get remove --purge --yes $PAQUETS_RANDOM > /dev/null 2>&1
...
apt-get remove --purge --yes network-manager network-manager-gnome > /dev/null 2>&1

# Tant qu'on y est, on supprime des paquets inutiles voire gênants
# pour certains.
apt-get remove --yes $PAQUETS_A_SUPPRIMER > /dev/null 2>&1
...
apt-get install --no-install-recommends --yes --reinstall $PAQUETS_AUTRES > /dev/null 2>&1
...
# et seulement Gnome.
apt-get remove --purge --yes twm >/dev/null 2>&1
...
afficher "Configuration de PAM afin que seul lightdm (la fenêtre de login)" \
...
# Ensuite, dans le fichier "/etc/pam.d/lightdm" et lui seul, on va
# changer les instructions « @include » pour importer les fichiers
# "/etc/pam.d/common-*.AVEC-LDAP". Ainsi, lightdm sera la seule application
...
service nslcd start > /dev/null 2>&1
...
if [ -f "/etc/samba/smb.conf" ]; then
    ...
    service samba restart > /dev/null 2>&1
fi
...
restaurer_via_save "$LIGHTDM_CONF"

# Si le paramètre "greeter-session" est sur la valeur "lightdm-gtk-greeter"
# (ce qui est le cas sur Xubuntu) alors on mettra le paramètre
# "greeter-hide-users" sur false et sinon sur true. En effet, avec cette
# fenêtre d'accueil, si l'on cache la liste des utilisateurs qui se sont
# déjà connectés, alors le bouton "autre" n'apparaît plus et une fois
# qu'on a saisi un login incorrect on tombe sur la saisie du mot de passe
# mais on ne peut plus aller en arrière (impossible de revenir sur la
# saisie du login ???).
if [ "$PARAM_GREETER_SESSION" = "lightdm-gtk-greeter" ]; then
    PARAM_GREETER_HIDE_USER="false"
else
    PARAM_GREETER_HIDE_USER="true"
fi

# On édite le fichier de A à Z. On utilise les variables
